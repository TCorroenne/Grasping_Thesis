...
class Image:
    ...
    def zoom(self, factor, center=None):
        ...
        if center is None:
            ...
            new_shape = (int(self.img.shape[0] * factor), int(self.img.shape[1] * factor))
            left = max(0, min(center[1] - new_shape[1] // 2, self.img.shape[1] - new_shape[1]))
            top = max(0, min(center[0] - new_shape[0] // 2, self.img.shape[0] - new_shape[0]))
            right = left + new_shape[1]
            bottom = top + new_shape[0]
            self.img = self.img[top:bottom, left:right].copy()
            self.img = resize(self.img, orig_shape, mode='symmetric', preserve_range=True).astype(self.img.dtype)
    ...
    def symmetry(self, integer):
        """
        Add symmetry to the image. (1: up-down, 2: left-right, 3: up-down and left-right)
        """
        if integer == 1:
            ...
            self.img = np.flipud(np.fliplr(self.img))
...
class DepthImage(Image):
    ...
    def normalise(self):
        ...
        """
        if self.img.max() >= 100: # If the image is in mm, convert to meters
            self.img = self.img / 1000.0
